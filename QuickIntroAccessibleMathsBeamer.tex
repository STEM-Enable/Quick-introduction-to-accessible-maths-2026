...
\author{Emma Cliffe}
\date{August 25, 2026}
\logo{\includegraphics{MASH University of Bath - tiny.png}}
...
maths\footnote<.->[frame]{I will use the word maths as a shorthand for
  all mathematical and statistical content used across any STEM degree.
...
\begin{frame}{The legal}
\phantomsection\label{the-legal}
Accessibility is a legal requirement\ldots{} I am not a
lawyer\footnote<.->[frame]{Legislation is cited for awareness only and
  this does not constitute legal advice. Specific legal questions should
  be referred to your University legal team.}. Seek advice.
...
\begin{frame}{But we should care anyway\ldots{}}
\phantomsection\label{but-we-should-care-anyway}
...
\begin{frame}{Structured text can be unexpectedly inaccessible}
\phantomsection\label{structured-text-can-be-unexpectedly-inaccessible}
\begin{figure}[H]

{\centering \includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio]{./Figs/Acrobat-thumbnail.png}
...
\begin{frame}{Could we solve it by\ldots?}
\phantomsection\label{could-we-solve-it-by}
Can we not solve this by ensuring that the PDF is accessible?
Unfortunately, the answer is currently, not yet\footnote<.->[frame]{Work
  is ongoing to add encoding within a new PDF standard (PDF/UA-2 and
...
{\centering \includegraphics[width=0.4\linewidth,height=\textheight,keepaspectratio]{./Figs/reflowing.png}
...
\begin{frame}{Structure + software}
\phantomsection\label{structure-software}
...
\begin{frame}[allowframebreaks]{So, job done?}
\phantomsection\label{so-job-done}
Afraid not\ldots{}
...
\item
  Clear print\footnote<.->[frame]{Adhering as closely as possible to
    RNIB clear print guidelines} PDF is selected most often by disabled
  students in the Department of Mathematical Sciences at the University
  of Bath\footnote<.->[frame]{Last year we had to assist an author to
    create a printable PDF for a disabled student after the author had
    made a whole (and completely technically accessible) course using
    ProTeXt - but couldn't get PDF working and the HTML produced poor
    print output\ldots{}}.
\end{itemize}
\end{frame}
...
\begin{frame}{Word/PowerPoint 365}
\phantomsection\label{wordpowerpoint-365}
\begin{itemize}
\tightlist
\item
...
\begin{frame}{Word/PowerPoint transforms and tools}
\phantomsection\label{wordpowerpoint-transforms-and-tools}
\begin{itemize}
\tightlist
\item
...
\begin{frame}{HTML (WCAG 2.2 AA) + MathJax!}
\phantomsection\label{html-wcag-2.2-aa-mathjax}
\begin{itemize}
\item
...
\begin{frame}[allowframebreaks]{Scientific document preparation systems}
\phantomsection\label{scientific-document-preparation-systems}
\begin{itemize}
\tightlist
\item
  Attempt to transform the LaTeX\footnote<.->[frame]{It is \textbf{not
    possible} to convert LaTeX to html in the general case. It is easy
    to `fall out' of the convertible subset without realising or knowing
    why, the addition of a package, change of preamble order, use of a
    seemingly innocent command\ldots{}} to HTML with MathJax using
...
\begin{frame}{Non-text elements}
\phantomsection\label{non-text-elements}
\begin{itemize}
\tightlist
\item
  Graphical elements
  ...
  \item
    \href{https://www.learningapps.co.uk/moodle/xertetoolkits/play.php?template_id=3023\#page1}{Complex
    images - making sense for accessibility}
  \item
    ...
    \tightlist
    \item
      Poet Image Description Training Tool, Image description guidelines
      and Sample book
    \item
